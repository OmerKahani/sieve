...
workloads = {
    ...
    "rabbitmq-operator": {
        ...
        .wait(50),
        "scaleup": test_framework.new_built_in_workload()
        .cmd("kubectl apply -f test-rabbitmq-operator/test/rmqc-1.yaml").wait_for_pod_status("rabbitmq-cluster-server-0", common.RUNNING)
        .cmd("kubectl patch RabbitmqCluster rabbitmq-cluster --type merge -p='{\"spec\":{\"replicas\":2}}'").wait_for_pod_status("rabbitmq-cluster-server-1", common.RUNNING)
        .wait(50),
    },
    "mongodb-operator": {
        "recreate": test_framework.new_built_in_workload()
